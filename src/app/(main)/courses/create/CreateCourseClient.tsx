...
import { CreateCourseFormData } from '@/services/courseServices';
...
const CreateCourseClient: React.FC = () => {
    ...
    const onFinish = async (values: CreateCourseFormData) => {
        if (!values.portada || values.portada.size === 0) {
            api.error({
                message: 'Error de Validación',
                description: 'Por favor, suba una imagen de portada para el curso.',
            });
            return;
        }
        const actualFile = Array.isArray(values.portada)
            ? values.portada[0]?.originFileObj
            : (values.portada as any)?.originFileObj;

        if (!actualFile) {
             api.error({ 
                message: 'Error de Validación',
                description: 'La imagen de portada no es válida o no se pudo procesar.',
            });
            return;
        }
        ...
        createCourseMutation.mutate(courseDataToSend);
    };
    ...
    const reorderModules = (currentIndex: number, newIndex: number) => {
        const currentModules = form.getFieldValue('modulos');
